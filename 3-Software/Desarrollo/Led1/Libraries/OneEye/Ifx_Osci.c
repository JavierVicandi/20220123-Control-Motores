...
static void Ifx_Osci_handleControlRequest(Ifx_Osci* osci);
void Ifx_Osci_setSamplingIntervalPrivate(Ifx_Osci* osci, sint16 interval);
...
pchar Ifx_Osci_getDataTypeName(Ifx_Osci_DataType type)
{
    ...
    case Ifx_Osci_DataType_Align32Bit:
    default:
        result = "Unknown";
        break;
    }
...
boolean Ifx_Osci_isEdgeDetected(Ifx_Osci* osci)
{
    ...
    case Ifx_Osci_TriggerEdge_falling:
        edgeDetected = (triggerPtr->previousValue > triggerPtr->level) && (value <= triggerPtr->level);
        break;
    case Ifx_Osci_TriggerEdge_align32Bit:
        break;
    }
...
#pragma warning 560 // W560: truncation at implicit conversion to type "unsigned short int" 13/12/2024 16:49:44
// Code which would generate warning 560.
...
void Ifx_Osci_arm(Ifx_Osci* osci)
{
    Ifx_Osci_Trigger* triggerPtr = &osci->trigger;
    osci->writeIndex  = 0;
    osci->readIndex   = 0;

    osci->trigger.previousValue = Ifx_Osci_valueGet(&osci->channels[osci->trigger.source]);
...
boolean Ifx_Osci_dataTransferDone(Ifx_Osci* osci)
{
...
static void Ifx_Osci_stepAutomatic(Ifx_Osci* osci)
{
    ...
    if (triggerPtr->state == Ifx_Osci_TriggerState_armed)
    {
        ...
        case Ifx_Osci_TriggerState_triggered:
            ...
            {
                triggerPtr->state = Ifx_Osci_TriggerState_dataTransfer;
                osci->stateChanged  = TRUE;
            }

            break;
        case Ifx_Osci_TriggerState_dataTransfer:
        	if (Ifx_Osci_dataTransferDone(osci))
        	{
        ...
        case Ifx_Osci_TriggerState_stopped:
            ...
            break;
        case Ifx_Osci_TriggerState_align32Bit:
        default:
            break;
        }
...
static void Ifx_Osci_stepInverted(Ifx_Osci* osci)
{
    ...
    if (triggerPtr->state == Ifx_Osci_TriggerState_triggered)
    {
    	triggerPtr->state = Ifx_Osci_TriggerState_stopped;
        osci->stateChanged  = TRUE;
...
{
	Ifx_Osci_handleControlRequest(osci);
    ...
    case Ifx_Osci_TriggerMode_align32Bit:
        break;
    }
...
boolean Ifx_Osci_associateSignalToChannel(Ifx_Osci* osci, Ifx_Osci_SignalId signalId, Ifx_Osci_ChannelId channelId)
{
    ...
    {
        if (signalId == IFX_OSCI_NO_SIGNAL)
        {
            osci->channels[channelId].signal = NULL_PTR;
            return TRUE;
        }
        else if (osci->signalList[signalId].id == IFX_OSCI_NO_SIGNAL)
        {
            return FALSE;
        }
...
Ifx_Osci_SignalId Ifx_Osci_addSignal(Ifx_Osci* osci, pchar name, Ifx_Osci_DataType type, const void* source, sint16 qFormatShift)
{
    ...
    if (osci->usedSignals < IFX_OSCI_CFG_MAX_NUM_OF_SIGNALS)
    {
        ...
        signalPtr->type         = type;
        signalPtr->getFctPtr    = getFctPtrArray[signalPtr->type];
...
boolean Ifx_Osci_init(Ifx_Osci* osci, Ifx_Osci_Config* config)
{
    ...
    IFX_ASSERT(IFX_VERBOSE_LEVEL_ERROR, sizeof(Ifx_Osci_Trigger) == IFX_OSCI_TRIGGER_STRUCT_SIZE);
    IFX_ASSERT(IFX_VERBOSE_LEVEL_ERROR, sizeof(Ifx_Osci_ControlInterface) == IFX_OSCI_CONTROLINTERFACE_STRUCT_SIZE);
    IFX_ASSERT(IFX_VERBOSE_LEVEL_ERROR, sizeof(Ifx_Osci_OneEye) == IFX_OSCI_ONEEYE_STRUCT_SIZE);
    IFX_ASSERT(IFX_VERBOSE_LEVEL_ERROR, sizeof(Ifx_Osci_Header) == IFX_OSCI_HEADER_STRUCT_SIZE);
    IFX_ASSERT(IFX_VERBOSE_LEVEL_ERROR, offsetof(Ifx_Osci, checkMark) == IFX_OSCI_OSCI_STRUCT_OFFSET_OF_CHECKMARK);

    result                 = TRUE;
    osci->oneEyeOsci.Version.major = IFX_OSCI_VERSION_MAJOR;
    osci->oneEyeOsci.Version.minor = IFX_OSCI_VERSION_MINOR;
    osci->oneEyeOsci.Version.patch = IFX_OSCI_VERSION_PARCH;
    ...
    osci->maxNumOfSamples  = IFX_OSCI_CFG_NUM_OF_SAMPLES;
    osci->remoteControlled = FALSE;
    osci->dataTransfered = FALSE;

    Ifx_Osci_clearControldata(&osci->controlInterface);
	...
	Ifx_Osci_setSamplingIntervalPrivate(osci, 1);
    ...
    uint32 i;
    osci->checkMark = 0;
    for (i=offsetof(Ifx_Osci, header);i<offsetof(Ifx_Osci, checkMark); i++)
    {
    ...
    strncpy((char *)&osci->oneEyeOsci.marker, IFX_OSCI_MARKER, sizeof(osci->oneEyeOsci.marker));

    return result;
}


boolean Ifx_Osci_setDepthPrivate(Ifx_Osci* osci, uint32 depth)
{
...
void Ifx_Osci_setSamplingIntervalPrivate(Ifx_Osci* osci, sint16 interval)
{
    osci->samplingInterval    = __saturate(interval, 1, 256);
    osci->samplingIntervalTmp = __saturate(interval, 1, 256);
...
void Ifx_Osci_handleControlRequest(Ifx_Osci* osci)
{
	if(osci->controlInterface.updateRequest == 1)
	{
		...
		if (osci->controlInterface.enable != 0)
		{
			...
			if (osci->enabled != enabled)
			{
				...
				osci->trigger.state = Ifx_Osci_TriggerState_stopped;
			    if (osci->enabled)
			    {
		...
		{
			if (osci->trigger.state == Ifx_Osci_TriggerState_stopped)
			{
				osci->trigger.arm = TRUE;
			}
		}
		...
		{
			if (osci->trigger.state == Ifx_Osci_TriggerState_armed)
			{
		...
		{
			if (osci->trigger.state == Ifx_Osci_TriggerState_dataTransfer)
			{
		...
		{
		    Ifx_Osci_TriggerMode mode = osci->controlInterface.triggerMode & 0x3;
		...
		if (osci->controlInterface.setBufferDepth != 0)
		{
			...
			{
				Ifx_Osci_setDepthPrivate(osci, bufferDepth);
				if (osci->trigger.state != Ifx_Osci_TriggerState_stopped)
				{
					osci->trigger.state = Ifx_Osci_TriggerState_stopped;
					osci->trigger.arm = TRUE;
		...
		{
			uint16 samplingInterval =  __saturate(osci->controlInterface.samplingInterval, 1, 256);
			if (osci->samplingInterval != samplingInterval)
			{
				Ifx_Osci_setSamplingIntervalPrivate(osci, samplingInterval);
				if (osci->trigger.state != Ifx_Osci_TriggerState_stopped)
				{
					osci->trigger.state = Ifx_Osci_TriggerState_stopped;
					osci->trigger.arm = TRUE;
		...
		{
		    osci->trigger.source = __saturate(osci->controlInterface.triggerSource, 0, IFX_OSCI_CFG_MAX_NUM_OF_CHANNELS - 1);
		    osci->trigger.previousValue = Ifx_Osci_valueGet(&osci->channels[osci->trigger.source]);
...
float32 Ifx_Osci_valueGet_Unknown(const Ifx_Osci_Signal* signal){return 0.0;}
float32 Ifx_Osci_valueGet_Float32(const Ifx_Osci_Signal* signal){return *((const float32*)signal->source);}
float32 Ifx_Osci_valueGet_SInt16(const Ifx_Osci_Signal* signal){return (float32)(*(const sint16*)signal->source);}
float32 Ifx_Osci_valueGet_UInt16(const Ifx_Osci_Signal* signal){return (float32)(*(const uint16*)signal->source);}
float32 Ifx_Osci_valueGet_SInt32(const Ifx_Osci_Signal* signal){return (float32)(*(const sint32*)signal->source);}
float32 Ifx_Osci_valueGet_UInt32(const Ifx_Osci_Signal* signal){return (float32)(*(const uint32*)signal->source);}
float32 Ifx_Osci_valueGet_FixPoint16(const Ifx_Osci_Signal* signal){return __fixpoint_to_float32((*(const sfract*)signal->source), signal->qFormatShift);}
float32 Ifx_Osci_valueGet_FixPoint32(const Ifx_Osci_Signal* signal){return __fixpoint_to_float32((*(const fract*)signal->source), signal->qFormatShift);}
float32 Ifx_Osci_valueGet_SInt8(const Ifx_Osci_Signal* signal){return (float32)(*(const sint8*)signal->source);}
float32 Ifx_Osci_valueGet_UInt8(const Ifx_Osci_Signal* signal){return (float32)(*(const uint8*)signal->source);}
float32 Ifx_Osci_valueGet_boolean(const Ifx_Osci_Signal* signal){return (float32)(*(boolean*)signal->source);}
float32 Ifx_Osci_valueGet_SInt64(const Ifx_Osci_Signal* signal){return (float32)(*(const sint64*)signal->source);}
float32 Ifx_Osci_valueGet_UInt64(const Ifx_Osci_Signal* signal){return (float32)(*(const uint64*)signal->source);}
float32 Ifx_Osci_valueGet_FctPtr_Float32(const Ifx_Osci_Signal* signal){return ((const float32 (*)(void))signal->source)();}
float32 Ifx_Osci_valueGet_FctPtr_SInt16(const Ifx_Osci_Signal* signal){return (float32)((const sint16 (*)(void))signal->source)();}
float32 Ifx_Osci_valueGet_FctPtr_UInt16(const Ifx_Osci_Signal* signal){return (float32)((const uint16 (*)(void))signal->source)();}
float32 Ifx_Osci_valueGet_FctPtr_SInt32(const Ifx_Osci_Signal* signal){return (float32)((const sint32 (*)(void))signal->source)();}
float32 Ifx_Osci_valueGet_FctPtr_UInt32(const Ifx_Osci_Signal* signal){return (float32)((const uint32 (*)(void))signal->source)();}
float32 Ifx_Osci_valueGet_FctPtr_FixPoint16(const Ifx_Osci_Signal* signal){return (float32)__fixpoint_to_float32(((const sfract (*)(void))signal->source)(), signal->qFormatShift);}
float32 Ifx_Osci_valueGet_FctPtr_FixPoint32(const Ifx_Osci_Signal* signal){return (float32)__fixpoint_to_float32(((const fract (*)(void))signal->source)(), signal->qFormatShift);}
float32 Ifx_Osci_valueGet_FctPtr_SInt8(const Ifx_Osci_Signal* signal){return (float32)((const sint8 (*)(void))signal->source)();}
float32 Ifx_Osci_valueGet_FctPtr_UInt8(const Ifx_Osci_Signal* signal){return (float32)((const uint8 (*)(void))signal->source)();}
float32 Ifx_Osci_valueGet_FctPtr_boolean(const Ifx_Osci_Signal* signal){return (float32)((const boolean (*)(void))signal->source)();}
float32 Ifx_Osci_valueGet_FctPtr_SInt64(const Ifx_Osci_Signal* signal){return (float32)((const sint64 (*)(void))signal->source)();}
float32 Ifx_Osci_valueGet_FctPtr_UInt64(const Ifx_Osci_Signal* signal){return (float32)((const uint64 (*)(void))signal->source)();}


float32 (* getFctPtrArray[])(const Ifx_Osci_Signal* source) =
{
           Ifx_Osci_valueGet_Unknown,
           Ifx_Osci_valueGet_Float32,
           Ifx_Osci_valueGet_SInt16,
           Ifx_Osci_valueGet_UInt16,
           Ifx_Osci_valueGet_SInt32,
           Ifx_Osci_valueGet_UInt32,
           Ifx_Osci_valueGet_FixPoint16,
           Ifx_Osci_valueGet_FixPoint32,
           Ifx_Osci_valueGet_SInt8,
           Ifx_Osci_valueGet_UInt8,
           Ifx_Osci_valueGet_boolean,
           Ifx_Osci_valueGet_SInt64,
           Ifx_Osci_valueGet_UInt64,
           Ifx_Osci_valueGet_FctPtr_Float32,
           Ifx_Osci_valueGet_FctPtr_SInt16,
           Ifx_Osci_valueGet_FctPtr_UInt16,
           Ifx_Osci_valueGet_FctPtr_SInt32,
           Ifx_Osci_valueGet_FctPtr_UInt32,
           Ifx_Osci_valueGet_FctPtr_FixPoint16,
           Ifx_Osci_valueGet_FctPtr_FixPoint32,
           Ifx_Osci_valueGet_FctPtr_SInt8,
           Ifx_Osci_valueGet_FctPtr_UInt8,
           Ifx_Osci_valueGet_FctPtr_boolean,
           Ifx_Osci_valueGet_FctPtr_SInt64,
           Ifx_Osci_valueGet_FctPtr_UInt64,
};
